...
class PSNRLoss(nn.Module):
    def __init__(self, max_value=2.0, eps=1e-8):
        super(PSNRLoss, self).__init__()
        self.max_value = max_value
        self.eps = eps
    ...
    def forward(self, signal, reference):
        """
        Negative PSNR loss for normalized I/Q signals.

        signal/reference shape: [B, 2, T]
        I and Q are normalized to [-1, 1], so max_value=2.0.
        """
        mse = torch.mean((signal - reference) ** 2, dim=(1, 2))

        psnr = 10 * torch.log10((self.max_value ** 2) / (mse + self.eps))
...
def calculate_psnr(signal, reference, max_value=2.0, eps=1e-8):
    """
    PSNR for normalized I/Q signals.

    signal/reference shape: [B, 2, T]
    I and Q are normalized to [-1, 1], so max_value=2.0.
    MSE is averaged over channel and time dimensions.
    """
    mse = torch.mean((signal - reference) ** 2, dim=(1, 2))

    psnr = 10 * torch.log10((max_value ** 2) / (mse + eps))
    return psnr
